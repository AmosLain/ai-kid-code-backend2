...
app.post('/generate', async (req, res) => {
  ...
  if (!prompt) {
    return res.json({
      code: '<h2 style="color:red; text-align:center;">Error: Prompt is missing</h2>'
    });
  }
  ...
  try {
    ...
    const errMsg = (error.response && error.response.data && error.response.data.error && error.response.data.error.message) || error.message || 'Unknown error';
    return res.json({
      code: ''
